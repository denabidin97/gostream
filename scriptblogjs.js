// ==========================
// GOSTREAM FINAL SCRIPT
// Versi aman + Adsterra
// Jadwal otomatis dari weekendsch.pages.dev
// ==========================

// --- Popup Adsterra (aktif saat klik pertama) ---
document.addEventListener("click", function adsterraPopupOnce() {
  const adScript = document.createElement("script");
  adScript.src = "//pl27844430.effectivegatecpm.com/8d/71/09/8d71097ce94b5dcce652f5da31899578.js";
  adScript.async = true;
  document.body.appendChild(adScript);
  console.log("✅ Adsterra popup aktif");
  document.removeEventListener("click", adsterraPopupOnce);
});

// --- Load channels.json (optional, jika kamu punya channel tetap) ---
async function loadChannels() {
  const container = document.querySelector("#live-tv #content");
  if (!container) return;
  container.innerHTML = "<p>Loading channels...</p>";
  try {
    const res = await fetch("https://govoet.pages.dev/channels.json");
    const channels = await res.json();
    container.innerHTML = "";
    channels.forEach(ch => {
      const div = document.createElement("div");
      div.className = "channel-container";
      div.innerHTML = `
        <div class='logo-container'><img src='${ch.logo}' class='logo'></div>
        <div class='info-container'><h3>${ch.name}</h3><p>${ch.status}</p></div>
      `;
      div.addEventListener("click", () => {
        loadVideo(ch.url);
      });
      container.appendChild(div);
    });
  } catch (e) {
    container.innerHTML = "<p>⚠️ Failed to load channels.</p>";
  }
}

// --- Load events dari JSON jadwal ---
async function loadEvents() {
  const container = document.querySelector("#live-event #content");
  if (!container) return;
  container.innerHTML = "<p>Loading events...</p>";
  try {
    const res = await fetch("https://weekendsch.pages.dev/sch/schedulegvt.json");
    const data = await res.json();
    container.innerHTML = "";

    // urutkan jadwal berdasarkan waktu
    data.sort((a, b) => new Date(a.match_date + ' ' + a.match_time) - new Date(b.match_date + ' ' + b.match_time));

    data.forEach(event => {
      const btn = document.createElement("div");
      btn.className = "event-container";
      btn.innerHTML = `
        <div class="event-header">
          <strong>${event.league}</strong>
        </div>
        <div class="event-details">
          <span>${event.team1.name}</span> vs <span>${event.team2.name}</span>
          <div>${event.kickoff_date} ${event.kickoff_time}</div>
        </div>
      `;
      btn.addEventListener("click", () => {
        const url = event.servers && event.servers[0]?.url ? event.servers[0].url : "";
        loadVideo(url);
      });
      container.appendChild(btn);
    });

    if (!data.length) {
      container.innerHTML = "<p>No schedule available.</p>";
    }
  } catch (e) {
    console.error("Error load jadwal:", e);
    container.innerHTML = "<p>⚠️ Gagal memuat jadwal.</p>";
  }
}

// --- Fungsi untuk memuat video di iframe ---
function loadVideo(url) {
  const iframe = document.getElementById("video-iframe");
  const placeholder = document.getElementById("video-placeholder");
  const countdown = document.getElementById("video-countdown");

  if (!iframe) return;

  if (url && url !== "about:blank") {
    iframe.src = url;
    iframe.style.display = "block";
    placeholder.style.display = "none";
    countdown.style.display = "none";
  } else {
    iframe.src = "about:blank";
    iframe.style.display = "none";
    placeholder.style.display = "block";
    countdown.style.display = "block";
  }
}

// --- Fungsi ganti tab sidebar ---
function switchContent(id) {
  document.querySelectorAll(".sidebar-content").forEach(div => {
    div.classList.remove("active");
  });
  const target = document.getElementById(id);
  if (target) target.classList.add("active");
}

// --- Auto refresh jadwal setiap 10 menit ---
setInterval(loadEvents, 10 * 60 * 1000);

// --- Inisialisasi saat halaman dibuka ---
document.addEventListener("DOMContentLoaded", () => {
  console.log("🚀 GOSTREAM script aktif");
  loadEvents();
  loadChannels();
});
